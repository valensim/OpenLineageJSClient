...
const {
  DatasetFacetsBuilder, ColumnLineage, Field, Transformation, Item, Schema,
  SchemaDatasetFacetFields, DataSource, DataQualityAssertions, Assertion,
  PreviousIdentifier, LifecycleStateChange, Storage, Symlinks, Identifier,
  Version
} = require("../src/facets/DatasetFacets");
const {TransformationType, FieldTransformationType} = require("../src/types");
const {OutputDatasetFacets, OutputStatistics} = require(
	"../src/facets/OutputDatasetFacets");
const {InputDatasetFacets, DataQualityMetrics, ColumnMetrics} = require(
...
describe('RunEvent', () => {
  it('should comply with the OpenLineage schema', async () => {
	...
	const runEvent = new RunEventBuilder(new Date().toISOString(), producer,
	...
	const jobEvent = new JobEventBuilder(new Date().toISOString(), producer, schemaURL)
	.setJob(job)
	.setInputs([inputDataset])
	.setOutputs([outputDataset])
	.build()
	...
	const datasetEvent = new DatasetEventBuilder(new Date().toISOString(), producer, schemaURL).setDataset(inputDataset).build()
